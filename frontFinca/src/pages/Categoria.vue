  ...
  <div style="background-color: white;">
    <div style="margin: 45px;">
      <div style="display:flex; justify-content: space-between; align-items: center;">
        <h2 style="color: orange; margin: 0;">Tipos de categoría</h2>
        <q-btn 
      ...
      <q-dialog v-model="fixed" :backdrop-filter="'blur(4px) saturate(150%)'" transition-show="rotate" transition-hide="rotate" persistent>
        <q-card>
          <q-card-section>
            <div class="text-h6">{{ b ? "Editar tipo de categoria" : "Guardar tipo categoria" }}</div>
          </q-card-section>

          <q-separator />

          <q-card-section style="max-height: 50vh" class="scroll">
            <q-input filled v-model="nombre" label="Nombre tipo cate" :dense="dense" />
          ...
          <q-card-actions align="right">
            ...
            <q-btn flat label="Guardar" color="primary" @click="guardarCategoria" />
          </q-card-actions>
        </q-card>
      </q-dialog>
...
onBeforeMount(async () => {
  ...
  rows.value = categoriaStore.categorias
})
...
async function guardarCategoria() {
  ...
  if (b.value) {
    // Editar
    const res = await categoriaStore.editarCategoria(id.value, {
      descripcion: descripcion.value,
      nombre: nombre.value
    })
    if (res) {
      Notify.create({ type: 'positive', message: 'Categoría editada con éxito.' })
      await categoriaStore.listarCategorias()
      rows.value = categoriaStore.categorias
      fixed.value = false
    }
    ...
    // Crear
    const res = await categoriaStore.crearCategoria({
      descripcion: descripcion.value,
      nombre: nombre.value
    })
    if (res) {
      Notify.create({ type: 'positive', message: 'Categoría creada con éxito.' })
      await categoriaStore.listarCategorias()
      rows.value = categoriaStore.categorias
      fixed.value = false
    }
...
async function eliminarFicha() {
  if (id.value) {
    const res = await categoriaStore.desactivarCategoria(id.value)
    if (res) {
      await categoriaStore.listarCategorias()
      rows.value = categoriaStore.categorias
      confirm.value = false
